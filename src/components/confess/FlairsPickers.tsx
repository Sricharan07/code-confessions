...
const VIBES = [
  { key: "vibe_coding", label: "Vibe Coding", desc: "No thoughts, just vibes" },
  { key: "deadline_panic", label: "Deadline Panic", desc: "Shipping in 5 mins" },
  { key: "drunk_coded", label: "Drunk Coded", desc: "Will fix tomorrow" },
  { key: "prod_on_fire", label: "Prod on Fire", desc: "Everything is burning" },
  { key: "4am_energy", label: "4AM Energy", desc: "Hallucinating code" },
] as const;

// Curated Verdict options
const VERDICTS = [
  { key: "still_broken", label: "Still Broken", desc: "Zero progress made" },
  { key: "nuked", label: "Nuked it", desc: "Deleted the folder" },
  { key: "solved", label: "Actually Solved", desc: "It works now" },
  { key: "cope_mode", label: "Cope Active", desc: "Works on my machine" },
  { key: "rebuilt", label: "Rebuilt", desc: "Started from scratch" },
] as const;

// Curated Plea options
const PLEAS = [
  { key: "innocent", label: "Plead Innocent", desc: "The AI lied to me" },
  { key: "deserve_it", label: "Deserve This", desc: "Blind copy-paste" },
  { key: "cooked", label: "Cooked AF", desc: "No further comments" },
] as const;
        ...
        {VIBES.map((v) => {
          ...
          return (
            <button
              ...
              className={`flex items-center py-1 px-3 rounded-full border text-[11px] font-semibold transition-all duration-150 scale-[1.0] hover:scale-[1.01] active:scale-[0.98] cursor-pointer ${
                isSelected
