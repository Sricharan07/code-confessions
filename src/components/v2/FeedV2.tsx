  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 flex flex-col">
        {tab === "following" && (!user || user.isGuest) ? (
          <div className="flex flex-col items-center justify-center h-64 px-4 text-center">
            <h2 className="text-2xl font-bold mb-2">Login to follow</h2>
            <p className="text-muted-foreground mb-6">Create an account to follow people and curate your feed.</p>
            <button className="bg-ink text-paper hover:opacity-90 font-bold py-3 px-6 rounded-full transition-colors">
              Log In / Sign Up
            </button>
          </div>
        ) : (
              ...
              return (
                <div key={p.id} className="p-4 border-b border-ink/10 hover:bg-ink/[0.02] transition-colors cursor-pointer flex gap-3">
                  <div className="w-10 h-10 shrink-0">
                    <img 
                      src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${p.author || 'anon'}`} 
                      alt="avatar" 
                      className="w-full h-full bg-ink/5 rounded-full border border-ink/10" 
                    />
                  </div>
