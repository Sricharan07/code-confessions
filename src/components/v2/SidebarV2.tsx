...
export function SidebarV2() {
  ...
  return (
    <>
      <div className="flex flex-col h-full justify-between py-4 w-full xl:w-[275px] px-2 md:px-4">
        <div>
          ...
          <nav className="flex flex-col items-start space-y-1">
            ...
            <div className="w-full mt-4">
              <button className="flex items-center justify-center gap-2 w-[85%] py-2.5 bg-hot hover:bg-hot/90 transition-colors rounded-full font-bold text-[15px] text-paper shadow-sm">
        ...
        <div className="mt-8">
          {user ? (
            <div className="flex items-center justify-between p-3 cursor-pointer hover:bg-ink/10 rounded-full transition-colors -ml-3">
              <div className="flex items-center gap-3">
                <img 
                  src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${user.username || 'anon'}`} 
                  alt="avatar" 
                  className="w-10 h-10 bg-ink/5 rounded-full border border-ink/10" 
                />
