...
export function AuthModalV2({ isOpen, onClose, onLogin }: AuthModalProps) {
  ...
  const handleRandomFill = async () => {
    const list = await getCuratedUsernameSuggestions(5);
    setSuggestions(list);
    setUsername(list[0]); // pre-fill the username field directly
  };
  ...
  useEffect(() => {
    if (activeTab === "signup") {
      getCuratedUsernameSuggestions(5)
        .then((list) => {
          setSuggestions(list);
          setUsername(list[0]); // Pre-fill the input field by default
        })
        .catch((err) => {
          console.error("Failed to load suggestions:", err);
          setUsername("Cursed_Dev_777");
        });
    } else {
      setUsername("");
    }
        ...
        {step === "auth" && (
          <div className="pt-1">
            ...
            <form onSubmit={handleAuthSubmit} className="space-y-4">
              {/* Username Input with Shuffle inside and suggestions below */}
              <div className="relative" ref={dropdownRef}>
                <label className="text-[12px] text-zinc-500 dark:text-zinc-400 font-bold block mb-1.5 uppercase tracking-wider font-mono">Username</label>
                ...
                <div className="relative flex items-center">
                  <input 
                    ...
                    className={`w-full bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl pl-4 py-3 text-[15px] font-semibold outline-none focus:border-zinc-400 dark:focus:border-zinc-600 transition-all text-zinc-900 dark:text-zinc-100 placeholder:text-zinc-400 ${activeTab === "signup" ? "pr-[96px]" : "pr-4"}`}
              ...
              <div>
                ...
                <input 
                  ...
                  className="w-full bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl px-4 py-3 text-[15px] font-semibold outline-none focus:border-zinc-400 dark:focus:border-zinc-600 transition-all text-zinc-900 dark:text-zinc-100 placeholder:text-zinc-400"
                  placeholder="••••••••"
                />
              </div>
